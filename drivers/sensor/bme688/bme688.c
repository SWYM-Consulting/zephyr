...
static int bme688_reg_read(struct bme688_data *data, uint8_t start, uint8_t *buf,
			   int size)
{
...
static int bme688_reg_write(struct bme688_data *data, uint8_t reg, uint8_t val)
{
...
void dbg_print_buf(uint8_t * buf, int len,int os)
{
	printk("<----BME688 Read Buf---->\n");
	for(int i = 0; i < len; i++){
			       ...
			       enum sensor_channel chan)
{
	...
	ret = bme688_reg_read(data, BME688_REG_FIELD0, buff, size);
	//dbg_print_buf(buff,size,BME688_REG_FIELD0);
	if (ret < 0) {
		return ret;
	}

	/*<---------------------------- Buffer Mem Map -------------------------->*/
		// [0] : new_data gas_measuring measuring		gas_meas_index
		// [1] : sub_meas_index_0
		// [2] : press_msb
		// [3] : press_lsb
		// [4] : press_xlsb
		...
		// [9] : hum_lsb 
		// [10]: gas_r<9:2>
		// [11]: gas_r<1:0> gas_valid_r heat_stab_r gas_range_r
		// [12]: sub_meas_index_1
		// [13]: pres_msb
		// [14]: press_lsb
	...
	gas_range = buff[16] & BME688_MSK_GAS_RANGE;
	...
	uint8_t buffer;
	bme688_reg_read(data,0x1D,&buffer,1);
	printk("Gas Meas Status: %d\n",buffer);
	/* Trigger the next measurement */
	ret = bme688_reg_write(data, BME688_REG_CTRL_MEAS,
			       BME688_CTRL_MEAS_VAL);
	bme688_reg_write(data, BME688_REG_CTRL_GAS_1,
...
static int bme688_read_compensation(struct bme688_data *data)
{
	uint8_t buff[BME688_LEN_COEFF_ALL];
	int err = 0;

	err = bme688_reg_read(data, BME688_REG_COEFF1, buff, BME688_LEN_COEFF1);
	if (err < 0) {
		return err;
	}

	err = bme688_reg_read(data, BME688_REG_COEFF2, &buff[BME688_LEN_COEFF1],
			      16);
	if (err < 0) {
		return err;
	}

	err = bme688_reg_read(data, BME688_REG_COEFF3,
	...
	// (37)[0x00] : res_heat_val
	// (38)[0x01] : 
	// (39)[0x02] : res_heat_range
	// (40)[0x03] : 
	// (41)[0x04] : 
...
{
	struct bme688_data *data = (struct bme688_data *)dev->data;
	int err;

	err = bme688_reg_read(data, BME688_REG_CHIP_ID, &data->chip_id, 1);
	if (err < 0) {
		return err;
	}

	if (data->chip_id == BME688_CHIP_ID) {
		printk("BME688 chip detected\n");
	} else {
		printk("Bad BME688 chip id 0x%x\n", data->chip_id);
		return -ENOTSUP;
	}

	err = bme688_read_compensation(data);
	if (err < 0) {
		return err;
	}

	err = bme688_reg_write(data, BME688_REG_CTRL_HUM, BME688_HUMIDITY_OVER);
	if (err < 0) {
		return err;
	}

	err = bme688_reg_write(data, BME688_REG_CONFIG, BME688_CONFIG_VAL);
	if (err < 0) {
		return err;
	}
	...
	err = bme688_reg_write(data, BME688_REG_RES_HEAT0,
			       bme688_calc_res_heat(data, BME688_HEATR_TEMP));
	if (err < 0) {
		return err;
	}

	err = bme688_reg_write(data, BME688_REG_GAS_WAIT0,
			       bme688_calc_gas_wait(BME688_HEATR_DUR_MS));
	if (err < 0) {
		return err;
	}

	err = bme688_reg_write(data, BME688_REG_CTRL_MEAS,
			       BME688_CTRL_MEAS_VAL);
	if (err < 0) {
		return err;
	}

	err = bme688_reg_write(data, BME688_REG_CTRL_GAS_1,
			       BME688_CTRL_GAS_1_VAL);
